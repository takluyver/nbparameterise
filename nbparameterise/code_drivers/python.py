...
def check_list(node: ast.List):
    return all([check_simple_value(n) for n in node.elts])

def check_fillable_node(node, path):
    if check_simple_value(node):
        return
...
def type_and_value(node, comments={}):
    ...
    if isinstance(node, ast.Constant):
        if isinstance(node.value, (bool, int, float, str)):
            return type(node.value), node.value, comment
    elif isinstance(node, ast.List):
